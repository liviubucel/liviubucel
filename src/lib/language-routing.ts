...
export function countryFromRequest(request: Request): string | null {
  // `request.cf.country` is the primary source in Cloudflare Workers/Astro.
  // CF-IPCountry is a defensive fallback when the zone's IP Geolocation or
  // visitor-location transform is enabled. We only consume the country code;
  // no IP, city, coordinates, or country value are persisted by this feature.
